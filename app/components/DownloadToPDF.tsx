...
export default function DownloadToPDF({ 
  ...
  const handleDownloadPDF = async () => {
    ...
    // 1. Setup off-screen rendering pipeline
    ...
    // 2. Pure print asset snapshot layout template
    const InvoiceTemplate = () => (
      ...
      >
        ...
        <div style={{ display: "flex", justifyContent: "space-between", backgroundColor: "#f8fafc", border: "1px solid #e2e8f0", padding: "16px", borderRadius: "12px" }}>
          <div>
        ...
        <table style={{ width: "100%", borderCollapse: "collapse" }}>
          <thead>
            <tr style={{ backgroundColor: `${systemSettings.billUI.themeColor}10` }} className="border-b-2 border-neutral-200 text-slate-500 font-bold text-[9px] uppercase">
              <th style={{ padding: "12px 8px", textAlign: "left" }}>LR Slip</th>
              <th style={{ padding: "12px 8px", textAlign: "left" }}>Asset</th>
              <th style={{ padding: "12px 8px", textAlign: "left" }}>Route / Destination</th>
        ...
        <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", width: "100%" }}>
          <div style={{ width: "288px" }} className="space-y-2">
            <div style={{ display: "flex", justifyContent: "space-between" }} className="font-medium text-slate-500 text-xs">
            ...
            {systemSettings.billUI.showGst && (
              <div style={{ display: "flex", justifyContent: "space-between" }} className="font-medium text-slate-400 text-[11px]">
                <span>Transport GST Sacc (18%):</span>
                <span className="font-mono">₹ {active.gst_amount.toLocaleString("en-IN")}</span>
              </div>
            )}
            <div style={{ color: systemSettings.billUI.themeColor, display: "flex", justifyContent: "space-between", borderTop: "1px dashed #cbd5e1", paddingTop: "8px" }} className="font-black text-base">
        ...
        <div className="mt-6 pt-4 border-t border-neutral-200">
          <div style={{ display: "flex", justifyContent: "space-between", backgroundColor: "#f8fafc", padding: "16px", borderRadius: "12px", border: "1px solid #e2e8f0" }}>
            <div>
            ...
            <div style={{ display: "flex", flexDirection: "column", justifyContent: "center", textAlign: "right" }}>
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }}><span className="text-[10px] font-bold text-slate-400 uppercase">A/c No:</span> <span className="font-mono font-bold text-slate-900">{systemSettings.bank_display_details.accountNumber || "0000 0000 0000"}</span></div>
              <div style={{ display: "flex", justifyContent: "flex-end", gap: "8px" }} className="mt-1"><span className="text-[10px] font-bold text-slate-400 uppercase">IFSC:</span> <span className="font-mono font-bold text-slate-900">{systemSettings.bank_display_details.ifscCode || "IFSC0000000"}</span></div>
            </div>
          </div>
        </div>

        <div style={{ display: "flex", justifyContent: "space-between", borderTop: "1px solid #f1f5f9", paddingTop: "24px" }} className="text-[10px] text-slate-400">
          <div style={{ width: "50%" }} className="leading-relaxed whitespace-pre-line text-slate-500">
    ...
    try {
      ...
      // OS Native Location Directory Picker Interface Verification
      if ("showSaveFilePicker" in window) {
        try {
          ...
          console.log("Save picker dismissed by the active user session.");
        }
      } else {
        pdf.save(fileName);
      }
    } catch (error) {
      console.error("PDF compiled thread system failure:", error);
    } finally {
